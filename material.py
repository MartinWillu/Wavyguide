...
# Ordinary: https://iopscience.iop.org/article/10.1143/JJAP.41.L465/pdf
# Extraordinary: N.A.Barboza - Improved Sellmeier equation for congruently grown lithium tantalate - 2009
class Lt(_Material):
    def __init__(self, axis, temperatureCelcius=20.):
        _Material.__init__(self)
        assert(axis in ['o', 'e'])
        ...
        if axis == 'o':
            ...
            self.A = 2.976
            lmA = 0.13798 + 3.2e-8*self.T**2
            self.lmA2 = lmA**2
            self.B = 0.5387 + 3.6e-4*self.T-1.2e-7*self.T**2
            lmB = 0.24028
            self.lmB2 = lmB**2
            self.C = -0.02337 - 4.3e-6*self.T + 3.5e-9*self.T**2
            self._eps_e_o = self._eps_e

    def _eps_o(self, wavelength):
        wl2 = (wavelength * 1.e-3)**2
        e = self.A + (self.B/(wl2-self.C)) - self.D*wl2
        return e

    def _eps_e(self, wavelength):
        wl2 = (wavelength * 1.e-3)**2
        e = 1 + self.A*wl2/(wl2 - self.lmA2) + \
            self.B*wl2/(wl2 - self.lmB2) + self.C*wl2
        return e
